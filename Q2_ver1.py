...
for i in range(1,len(temp)):
    ...
    temp[i] = temp[i].split(", ")
    details = {"crossing": temp[i][1], "gate_no": int(temp[i][2]), "time":[int(num) for num in temp[i][3].split(":")]}
...
while(True):
    ...
    if query==1:
        ...
        try:
            ...
            print("Please enter time in the correct format.\n")
        status = ""
        output = []
        ...
        for i in records:
            if i==student:
                for j in records[i]:
                    ...
                    output.append(tuple(j.values()))

                    if j["time"][0]<current_time[0]:
                        status = j["crossing"]

                    elif j["time"][0]==current_time[0] and j["time"][1]<current_time[1]:
                        status = j["crossing"]

                    elif j["time"][0]==current_time[0] and j["time"][1]==current_time[1] and j["time"][2]<=current_time[2]:
                        status = j["crossing"]

        if status=="ENTER":
            status = ("Student is inside the campus.")

        elif status=="EXIT":
            status = ("Student is not inside the campus.")
        ...
        for i in records:
            ...
            for j in records[i]:
                ...
                if start_time[0]<j["time"][0]<end_time[0]:
                    flag = True
                    
                elif start_time[0]==j["time"][0]<end_time[0]:
                    if start_time[1]<j["time"][1]:
                        flag = True

                    elif start_time[1]==j["time"][1]:
                        if start_time[2]<=j["time"][2]:
                            flag = True
                
                elif start_time[0]<j["time"][0]==end_time[0]:
                    if j["time"][1]<end_time[1]:
                        flag = True

                    elif j["time"][1]==end_time[1]:
                        if j["time"][2]<=end_time[2]:
                            flag = True
            
                elif start_time[0]==j["time"][0]==end_time[0]:
                    if start_time[1]<j["time"][1]<end_time[1]:
                        flag = True

                    elif start_time[1]==j["time"][1]<end_time[1]:
                        if start_time[2]<=j["time"][2]:
                            flag = True
                        ...
                        if start_time[2]<=j["time"][2]<=end_time[2]:
                            flag = True
                
                if flag:
                    new_time = []
                    for k in range(len(j["time"])):
                        if len(str(j["time"][k]))==1:
                            new_time.append("0"+str(j["time"][k]))

                        else:
                            new_time.append(str(j["time"][k]))

                    output.append(f"\n{i}, {j['crossing']}, {j['gate_no']}, {':'.join(new_time)}")
